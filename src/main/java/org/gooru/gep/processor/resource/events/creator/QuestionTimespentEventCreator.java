 ...
 *  
 */
public class QuestionTimespentEventCreator implements ResourceEventCreator {
	
	private static final String TOPIC_QUESTION_TIMESPENT = "org.gooru.da.sink.gep.question.timespent";
    private static final String QUESTION_TIMESPENT = "usage.question.timespent";
    ResourceEventObject queEvent = new ResourceEventObject();

    public QuestionTimespentEventCreator(ResourceEventObject queEvent) {
  ...
  //*************************************************************************************************************      
    ...
    private void sendDiscreteEventtoKafka(JSONObject discreteEvent) {
        ...
        try {
          KafkaMessagePublisher.getInstance().sendMessage2Kafka(TOPIC_QUESTION_TIMESPENT, dEvent);
          LOGGER.info("Successfully dispatched Question Timespent event..");
        } catch (Exception e) {
          LOGGER.error("Error while dispatching Question Timespent event.. ", e);
        }
      }
